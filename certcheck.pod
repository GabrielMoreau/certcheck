...
=head1 SYNOPSIS
 ...
 certcheck check [--verbose|-v] [--base|-b certbase] [--pub|-p certbase.pub.pem] [--key|-k certbase.key.pem] [--chain|-c certbase.chain.pem]
 certcheck text  [--verbose|-v] [--base|-b certbase] [--pub|-p certbase.pub.pem]
 certcheck txt   [--verbose|-v] [--base|-b certbase] [--pub|-p certbase.pub.pem]
 certcheck info  [--verbose|-v] [--base|-b certbase] [--pub|-p certbase.pub.pem]
 certcheck text  [--verbose|-v] [--sort|-s] certbase1.pub.pem certbase2.pub.pem certbase*.pub.pem
 certcheck txt   [--verbose|-v] [--sort|-s] certbase1.pub.pem certbase2.pub.pem certbase*.pub.pem
 certcheck info  [--verbose|-v] [--sort|-s] certbase1.pub.pem certbase2.pub.pem certbase*.pub.pem
 certcheck version
 certcheck help
...
If no option is passed, but several public certificates are on the command line,
then they are all processed with the C<info> option (or C<txt>, C<info>)).

 certcheck text certbase1.pub.pem certbase2.pub.pem certbase*.pub.pem

It is possible to have the dates of your certificates sorted by expiration date.
This makes it easier to get an overview of critical renewals.

In any case, monitoring with a tool such as Nagios will alert you
one week to 15 days before the end of a certificate's validity period.

 LANG=C certcheck text */*/certbase.pub.pem \
  | grep -E '^(Certificate:|Not After)' \
  | cut -f 2- -d ':' \
  | paste -d ' ' - - \
  | LANG=C sort -k 5 -k 2M -k3,4

The C<--sort> option does exactly that.
To be reliable, the output is always in C<LANG=C>.

 certcheck text --sort certbase1.pub.pem certbase2.pub.pem certbase*.pub.pem
